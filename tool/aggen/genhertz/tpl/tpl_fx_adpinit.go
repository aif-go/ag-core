...
var FxAdpInitImportsSetter = func(geni *types.GennerInfo) error {
	...
	//eg: import hzwhello "ag-layout-demo/internal/adpgen/hertz/hzwhello"
	geni.AddImport(lowerSvcName, fmt.Sprintf("%s/%s/%s/%s/%s", _module.PwdGoMod, "internal", "adpgen", "hertz", lowerSvcName))

	// util.TestLog("=======", _pkgInfo)
...
func init(){
	AddFxAdapterOpt({{ToLower $LServiceName}}.Fx{{$LServiceName}}HertzModule)
}
`

...
// TestStudentUpdateByPrimaryKeyIgnoreZeroValCols 覆盖 tm_student 的 UpdateByPrimaryKeyIgnoreZeroValCols（新拼写）：
// 与旧名 UpdateByPrimaryKeyIngoreZeroValCols 行为等价（旧名一行委托到新名），验证新名调用路径
func TestStudentUpdateByPrimaryKeyIgnoreZeroValCols(t *testing.T) {
	ctx := context.Background()
	studentDao := GetStudentRepository()
	...
	seed := &model.TmStudent{
		TenantId: 90014, StudentNo: "TEST014", Name: "新名学生",
		Address: "原地址014", Phone: "13800009014", ClassId: "T9", CardNo: "ORGCARD014",
		Score: decimal.NewFromFloat(66.6),
	}
	if _, err := studentDao.InsertOne(ctx, seed); err != nil {
		t.Fatalf("前置插入失败: %v", err)
	...
	t.Run("场景1:仅更新指定非零字段-其他字段保持不变", func(t *testing.T) {
		before, err := studentDao.FindByPrimaryKey(ctx, model.TmStudentPrimaryKey{TenantId: seed.TenantId, StudentNo: seed.StudentNo})
		if err != nil || before == nil {
			t.Fatalf("加载原实体失败: %v", err)
		}

		patch := &model.TmStudent{TenantId: seed.TenantId, StudentNo: seed.StudentNo, ClassId: "C8"}
		affected, err := studentDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, patch)
		if err != nil {
			t.Fatalf("UpdateByPrimaryKeyIgnoreZeroValCols 不期望错误: %v", err)
		}
		if affected != 1 {
			t.Errorf("期望影响 1 行，实际 %d", affected)
		}

		after, err := studentDao.FindByPrimaryKey(ctx, model.TmStudentPrimaryKey{TenantId: seed.TenantId, StudentNo: seed.StudentNo})
		if err != nil || after == nil {
			t.Fatalf("回查失败: %v", err)
		}
		if after.ClassId != "C8" {
		...
		if after.Name != before.Name || after.CardNo != before.CardNo || !after.Score.Equal(before.Score) {
			t.Errorf("零值字段不应被改动: %+v", after)
		}
	})

	t.Run("场景2:主键缺失-预期错误", func(t *testing.T) {
		_, err := studentDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, &model.TmStudent{Name: "x"})
		assertErrorContains(t, err, "primary key is required")
	})

	t.Run("场景3:更新不存在的主键-影响0行", func(t *testing.T) {
		affected, err := studentDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, &model.TmStudent{TenantId: 99999, StudentNo: "NOEXIST", ClassId: "C1"})
		if err != nil {
			t.Fatalf("更新不存在主键不期望错误: %v", err)
		}
		if affected != 0 {
...
// TestTeacherUpdateByPrimaryKeyIgnoreZeroValCols 覆盖 tm_teacher 的 UpdateByPrimaryKeyIgnoreZeroValCols（新拼写）：
// 与旧名 UpdateByPrimaryKeyIngoreZeroValCols 行为等价（旧名一行委托到新名），验证新名调用路径
func TestTeacherUpdateByPrimaryKeyIgnoreZeroValCols(t *testing.T) {
	ctx := context.Background()
	teacherDao := GetRepository()
	...
	seed := &model.TmTeacher{
		Id: 90014, Name: "新名教师", Address: "原地址014",
		Phone: "13900009014", ClassId: "T9", CardNo: "ORGCARD014",
		Salary: decimal.NewFromFloat(15000),
	}
	if _, err := teacherDao.InsertOne(ctx, seed); err != nil {
		t.Fatalf("前置插入失败: %v", err)
	...
	t.Run("场景1:仅更新指定非零字段-其他字段保持不变", func(t *testing.T) {
		...
		patch := &model.TmTeacher{Id: seed.Id, CardNo: "UPDCARD014", JpaVersion: before.JpaVersion}
		affected, err := teacherDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, patch)
		if err != nil {
			t.Fatalf("UpdateByPrimaryKeyIgnoreZeroValCols 不期望错误: %v", err)
		}
		if affected != 1 {
		...
		if after.CardNo != "UPDCARD014" {
			t.Errorf("card_no 未更新: %+v", after)
		}
		if after.Name != before.Name || !after.Salary.Equal(before.Salary) {
			t.Errorf("零值字段不应被改动: %+v", after)
		}
	})

	t.Run("场景2:主键缺失-预期错误", func(t *testing.T) {
		_, err := teacherDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, &model.TmTeacher{Name: "x", JpaVersion: ver(1)})
		assertErrorContains(t, err, "primary key is required")
	})

	t.Run("场景3:更新不存在的主键-影响0行", func(t *testing.T) {
		affected, err := teacherDao.UpdateByPrimaryKeyIgnoreZeroValCols(ctx, &model.TmTeacher{Id: 999999999, CardNo: "X", JpaVersion: ver(1)})
		if err != nil {
			t.Fatalf("更新不存在主键不期望错误: %v", err)
		}
		if affected != 0 {

...
**方法说明**：根据实体对象中的非零值字段构建查询条件，查询匹配的记录列表。

**索引规则**：
- **至少一个引导索引列必填**：任一索引的最左列（引导索引列）有值时才会放行查询；全部引导索引列为空时返回错误 `query not use any index`，避免全表扫描。
- **所有非零列参与过滤**：除主键外，所有非零字段（含索引列）都会作为等值 AND 条件拼入 WHERE。未激活索引的次要列也会参与过滤，结果会比只按索引列过滤更精确（可能更少）。

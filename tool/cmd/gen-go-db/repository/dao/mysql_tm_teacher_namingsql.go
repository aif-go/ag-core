...
const MYSQL_TmTeacher_FindByNameNadAddress = "SELECT name,address,phone,class_id FROM tm_teacher WHERE (name like CONCAT('%', @Name) AND address like CONCAT('%', @Address, '%'))"

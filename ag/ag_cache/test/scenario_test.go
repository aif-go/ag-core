...
func TestScenario_UserCache_CacheAside(t *testing.T) {
	...
	var dbCalls int
	var mu sync.Mutex
	users := mustCache[*User](t, dflt(), "users", func(ctx context.Context, key string) (*User, error) {
		mu.Lock()
		dbCalls++
		mu.Unlock()
		u, ok := db[key]
...
func TestScenario_ParamCache_ClearReload(t *testing.T) {
	...
	params := mustCache[*Param](t, dflt(), "params", func(ctx context.Context, key string) (*Param, error) {
		v, ok := paramStore[key]
		if !ok {
			return nil, ag_cache.ErrCacheMiss
		}
...
func TestScenario_ForceRefresh_SetAndDel(t *testing.T) {
	...
	var loaded int
	c := mustCache[string](t, dflt(), "cfg", func(ctx context.Context, key string) (string, error) {
		loaded++
		return "from-db", nil
	})
	reader, _ := ag_cache.GetCache[string](dflt(), "cfg") // 纯读视角（同实例，TryGet 不调 loader）
...
func TestScenario_NegativeCachePattern(t *testing.T) {
	...
	notExist := mustCache[bool](t, dflt(), "user-notexist", func(ctx context.Context, key string) (bool, error) {
...
func TestScenario_BasicTypeSerialization(t *testing.T) {
	...
	str := mustCache[string](t, dflt(), "s", func(ctx context.Context, key string) (string, error) { return "sv", nil })
	i64 := mustCache[int64](t, dflt(), "i64", func(ctx context.Context, key string) (int64, error) { return 42, nil })
	b := mustCache[bool](t, dflt(), "b", func(ctx context.Context, key string) (bool, error) { return true, nil })
	bytes := mustCache[[]byte](t, dflt(), "bytes", func(ctx context.Context, key string) ([]byte, error) { return []byte("bv"), nil })

	strReader, _ := ag_cache.GetCache[string](dflt(), "s")
	i64Reader, _ := ag_cache.GetCache[int64](dflt(), "i64")
	bReader, _ := ag_cache.GetCache[bool](dflt(), "b")
	bytesReader, _ := ag_cache.GetCache[[]byte](dflt(), "bytes")
...
func TestScenario_Probe_WithTryGet(t *testing.T) {
	...
	c := mustCache[string](t, dflt(), "probe", func(ctx context.Context, key string) (string, error) {
		return "uv", nil
	})

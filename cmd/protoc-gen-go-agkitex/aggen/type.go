...
import (
	...
	kgen "github.com/cloudwego/kitex/tool/internal_pkg/generator"
...
type PackageInfo struct {
	...
	*ServiceInfo                   // the target service
	...
	// add by houzw , plugins相关信息
	PluginName     string
	PluginSortName string
...
func (p *PackageInfo) toExternalGenPath(path string) string {
	if p.ExternalKitexGen == "" || !strings.Contains(path, kgen.KitexGenPath) {
		return path
	}
	parts := strings.Split(path, kgen.KitexGenPath)
...
type Task struct {
	...
	Path      string
	Text      string
	SetImport func(pkg *PackageInfo)
	*template.Template
	// Ext *APIExtension
}
